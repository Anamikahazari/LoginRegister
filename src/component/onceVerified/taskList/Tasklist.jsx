import React from 'react'

export default function Tasklist({taskList,setTask}) {
  console.log("inside tasklist", taskList);
  const removeTask = (index)=>{
    console.log("removing",index);
    console.log("tasklist",taskList);
    let arr = [...taskList];
    arr.splice(index,1);
    setTask(arr);
    console.log(taskList);

    // console.log("Tasklist",props.setTask(props.taskList));
    // console.log("new Tasklist", props.taskList);
    // props.taskList= props.taskList.filter((i) => i !== newList)
    // console.log(props.taskList);
  }
  return (
    <div>
      {taskList.map((Task, index) => {
        return <div key = {index} className='flex items-center'>
                <div className='bg-transparent text-black belowBorderLine p-3 '>{Task}</div>
                <div className='removeTask'  onClick={()=>{removeTask(index)}}><img src='data:image/png;base64,' className='w-3 h-3 bg-transparent cursor-pointer'></img>
              </div>
            </div>
      })}
    </div>
  )
}
